...
StateLasso::StateLasso() : Smach::state<StateLasso_Context>("lasso", N_("Cutout Tool"))
{
...
void StateLasso_Context::load_settings()
{
	try
	{
...
void StateLasso_Context::save_settings()
{
	try
	{
...
void StateLasso_Context::increment_id()
{
	String id(get_id());
	int number = 1;
	int digits = 0;
...
StateLasso_Context::StateLasso_Context(CanvasView *canvas_view) : canvas_view_(canvas_view),
...
void StateLasso_Context::UpdateUsePressure()
{
...
void StateLasso_Context::UpdateCreateAdvancedOutline()
{
...
bool StateLasso_Context::process_queue()
{
...
StateLasso_Context::new_bline(std::list<synfig::BLinePoint> bline, std::list<synfig::WidthPoint> wplist, bool loop_bline_flag, float radius)
{
	...
	if (get_auto_extend_flag() || get_auto_link_flag())
	{
		...
		if (start_duck)
			do
			{
				if (!(start_duck_value_desc = start_duck->get_value_desc()))
					break;
				if (loop_bline_flag)
					break; // loops don't extend anything
				if (!start_duck_value_desc.parent_is_value_node())
					break;
		...
		if (finish_duck)
			do
			{
				if (!(finish_duck_value_desc = finish_duck->get_value_desc()))
					break;
				if (loop_bline_flag)
					break;
				if (!finish_duck_value_desc.parent_is_value_node())
					break;
	...
	if (extend_start)
	{
		...
		if (extend_start_join_different)
			LinkableValueNode::Handle::cast_dynamic(source.value_node)->set_link("point", finish_duck_value_desc.get_value_node());
		else if (extend_start_join_same)
			LinkableValueNode::Handle::cast_dynamic(source.value_node)->set_link("point", synfigapp::ValueDesc(LinkableValueNode::Handle::cast_dynamic(start_duck_value_node_bline->list[target_offset + finish_duck_index].value_node), 0).get_value_node());
		return result;
	}
	...
	if (extend_finish)
	{
		...
		if (extend_finish_join_different)
			LinkableValueNode::Handle::cast_dynamic(source.value_node)->set_link("point", start_duck_value_desc.get_value_node());
		else if (extend_finish_join_same)
			LinkableValueNode::Handle::cast_dynamic(source.value_node)->set_link("point", synfigapp::ValueDesc(LinkableValueNode::Handle::cast_dynamic(finish_duck_value_node_bline->list[target_offset + start_duck_index].value_node), 0).get_value_node());
		return result;
	}

	if (join_start_no_extend)
		LinkableValueNode::Handle::cast_dynamic(value_node->list.front().value_node)->set_link("point", start_duck_value_desc.get_value_node());

	if (join_finish_no_extend)
		LinkableValueNode::Handle::cast_dynamic(value_node->list.back().value_node)->set_link("point", finish_duck_value_desc.get_value_node());
...

	{
		...
		if (layer2)
		{
			...
			if (value_node_wplist)
				value_node_wplist->set_member_canvas(get_canvas());
...
StateLasso_Context::new_region(std::list<synfig::BLinePoint> bline, synfig::Real radius)
{
	...
	if (!value_node_bline)
	{
		...
		while (iter != vertex_list.end())
		{
			...
			if (whole)
				min_index = max_index = (min_index + max_index) / 2;
					...
					else
					{
						...
						if (value_desc.get_value_node() != value_next.get_value_node())
						{
							...
							if ((vertex.get_vertex() - vertex_next.get_vertex()).mag_squared() < radius * radius)
							{
								...
								value_node = ValueNode_Composite::Handle::cast_dynamic(value_desc.get_value_node()->clone(value_desc.get_value_node()->get_parent_canvas()));
								value_node_next = ValueNode_Composite::Handle::cast_dynamic(value_next.get_value_node()->clone(value_next.get_value_node()->get_parent_canvas()));
...
void StateLasso_Context::refresh_ducks()
{
...
void StateLasso_Context::reverse_bline(std::list<synfig::BLinePoint> &bline)
{
	int i;
...
void StateLasso_Context::reverse_wplist(std::list<synfig::WidthPoint> &wplist)
{
	for (std::list<synfig::WidthPoint>::iterator iter = wplist.begin(); iter != wplist.end(); ++iter)
		iter->reverse();
}
...
void StateLasso_Context::fill_last_stroke()
{
	if (!last_stroke)
		return;
...
void StateLasso_Context::toggle_layer_creation()
{
